...
import { apiGetTaskById, apiUpdateTask } from "../../api/task.api";
...
const Task = () => {
  ...
  const getTaskById = () => {
    if (taskId) {
      const taskData = apiGetTaskById(Number(taskId));
      if (taskData) {
        setTask(taskData);
      } else {
        setNotFound(true);
      ...
      <div className="w-3/6 h-auto p-3 bg-white rounded-md border-dashed border border-purple-400">
        {notFound ? (
          <div className="w-full h-full flex flex-col justify-center items-center">
            ...
            />
            <span className="text-red-500 mt-1">Not found task</span>
          </div>
        ) : !loading ? (
          <>
            <h4 className="font-bold text-lg text-purple-500">Update Task</h4>
          ...
          <>
            <div className="w-full h-full flex flex-col justify-center items-center">
              <img
                ...
                className="w-12 h-12 aspect-auto"
              />
              <span className="text-purple-500 mt-1">Updating tasks...</span>
            </div>
          </>
        )}
      </div>
